...
namespace Binance.API.Csharp.Client
{
    public class BinanceClient : BinanceClientAbstract, IBinanceClient
    {
        ...
        public async Task<AccountSnapshot> GetAccountSnapshot(string type = "SPOT", DateTime? startTime = null, DateTime? endTime = null, long limit = 5, long recvWindow = 5000)
        {
            var args = $"type={type.ToUpper()}"
                 + (startTime.HasValue ? $"&startTime={startTime.Value.GetUnixTimeStamp()}" : "")
              + (endTime.HasValue ? $"&endTime={endTime.Value.GetUnixTimeStamp()}" : "")
                + $"&limit={limit}"
               + $"&timestamp={Utilities.GenerateTimeStamp(DateTime.Now.ToUniversalTime())}&recvWindow={recvWindow}";

            var result = await _apiClient.CallAsync<AccountSnapshot>(ApiMethod.GET, EndPoints.AccountSnapshot, true, args);
